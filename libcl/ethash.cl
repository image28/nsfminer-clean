...
// want to run this on cpu, and LD_PRELOAD a counter output into sizeof to see how many 
// this gets called. Is it called whenever these union types are accessed 
// Did that with this text editor a few years back, was calling strlen millions of
// times a minute, even when idle/not in use/minimized.... ( strlen's pretty fast though, 
// but still.
    ...
    ulong start_nonce, ulong target)
{
    ...
    const uchar thread_id = (uchar)(get_local_id(0) & 3);
    const uchar hash_id = (uchar)(get_local_id(0) >> 2);
    ...
    for(tid=0; tid < 4; tid++)
	{
		...
		do
		{
			const uchar lane_idx = hash_id2 + lane;
			#pragma unroll 8
			for (x = 0; x < 8; ++x)
				MIX(x);
